"""

FC - distance & fc - volume

Distance:
    ctx only
    SignificanceResult(statistic=-0.29691788598393354, pvalue=0.0)
    bstem to ctx
    SignificanceResult(statistic=0.03578973453054543, pvalue=4.9587562253731816e-08)
    bstem only
    SignificanceResult(statistic=-0.10602603578409776, pvalue=1.5629031651763996e-05)

Volume:
    ctx only
    SignificanceResult(statistic=-0.012799150931088054, pvalue=0.00029952554214543627)
    bstem to ctx
    SignificanceResult(statistic=0.6002207955870578, pvalue=0.0)
    bstem only
    SignificanceResult(statistic=0.649902188802195, pvalue=5.246664085375575e-199)

"""

#------------------------------------------------------------------------------
# Libraries
#------------------------------------------------------------------------------

import numpy as np
import pandas as pd
import seaborn as sns
from scipy.io import loadmat
import matplotlib.pyplot as plt
from scipy.stats import spearmanr
from palettable.colorbrewer.sequential import PuBuGn_9
from globals import path_fc, path_results, path_dist_size, n_brainstem

#------------------------------------------------------------------------------
# Region information file
#------------------------------------------------------------------------------

info = pd.read_csv(path_fc + 'region_info_Schaefer400.csv', index_col = 0) # 438 columns
# 1:58 brainstem # 59:67 diencephalic # 68:74 subcortical # 76:475 cortical # 476:483 subcortcal
idx_bstem = info.query("structure == 'brainstem'").index.values
idx_ctx = info.query("structure == 'cortex'").index.values
idx_bc = np.concatenate((idx_bstem, idx_ctx))

name_bstem = info.labels[idx_bstem]
names = info.labels[idx_bc]
bs_voxels = info.nvoxels[idx_bstem]

coor = info[['x', 'y', 'z']].values
coor = coor[idx_bc, :] # 458 by 3 - this is the coordination of parcels

#------------------------------------------------------------------------------
# Functional connectome (FC)
#------------------------------------------------------------------------------

fc_matlab = loadmat(path_fc + 'mean_corrcoeff_full.mat')
fc = fc_matlab['C_BSwithHO_mean']
fcsubj = fc_matlab['C_BSwithHO']
fcsubj = np.delete(fcsubj, 10, axis = 2) # remove bad subject in sc
fc = np.mean(fcsubj[idx_bc,:,:][:, idx_bc,:], axis = 2)
np.fill_diagonal(fc, 1) # for visualization

plt.figure(figsize = (15, 15))
sns.heatmap(fc,
            vmin = 0, vmax = 1,
            cmap = PuBuGn_9.mpl_colormap,
            xticklabels = False,
            yticklabels = False,
            cbar = False)
plt.axhline(y = n_brainstem,
            color = 'black',
            linewidth = 0.8)
plt.axvline(x = n_brainstem,
            color = 'black',
            linewidth = 0.8)
plt.savefig(path_results + 'heatmap_fc_whole_brain.png', dpi = 300)
plt.show()

np.fill_diagonal(fc, np.nan)

#------------------------------------------------------------------------------
# Distance
#------------------------------------------------------------------------------

to_drop = ['Sp_v', 'Sp_d', 'STh_subregion1_l',
           'STh_subregion2_l', 'HTH', 'STh_subregion2_r',
           'STh_subregion1_r', 'LG_l', 'LG_r', 'MG_l', 'MG_r',
           'L-Accumbens-area', 'R-Accumbens-area',
           'L-Amygdala', 'R-Amygdala', 'L-Hippocampus', 'R-Hippocampus',
           'L-Pallidum', 'R-Pallidum', 'L-Putamen', 'R-Putamen',
           'L-Caudate', 'R-Caudate', 'L-Thalamus-Proper', 'R-Thalamus-Proper',
           'L-Cereb-Ctx', 'R-Cereb-Ctx']

dist = pd.read_csv(path_dist_size + 'dist.csv', index_col = 0) # 'Unnamed: 0' becomes index
dist = dist.drop(index=to_drop, columns=to_drop)
dist = np.array(dist)
dist = dist.astype(float)
np.save(path_results + 'distance_whole_brain.npy', dist)

plt.figure(figsize = (15, 15))
sns.heatmap(dist,
            vmin = 0, vmax = np.max(dist),
            cmap = PuBuGn_9.mpl_colormap,
            xticklabels = False,
            yticklabels = False,
            cbar = False)
plt.axhline(y = n_brainstem,
            color = 'black',
            linewidth = 0.8)
plt.axvline(x = n_brainstem,
            color = 'black',
            linewidth = 0.8)
plt.savefig(path_results + 'distance_whole_brain.png', dpi = 300)
plt.show()

plt.show()
vol = pd.read_csv(path_dist_size + 'volsqrt.csv', index_col = 0) # vol by vol
vol = vol.drop(index = to_drop, columns=to_drop)
vol = np.array(vol)
vol = vol.astype(float)
vol = vol[:458,:]
np.save(path_results + 'volume_whole_brain.npy', vol)

plt.figure(figsize = (15, 15))
sns.heatmap(vol,
            vmin = 0, vmax = np.max(vol),
            cmap = PuBuGn_9.mpl_colormap,
            xticklabels = False,
            yticklabels = False,
            cbar = False)
plt.axhline(y = n_brainstem,
            color = 'black',
            linewidth = 0.8)
plt.axvline(x = n_brainstem,
            color = 'black',
            linewidth = 0.8)
plt.savefig(path_results + 'volume_whole_brain.png', dpi = 300)
plt.show()

#------------------------------------------------------------------------------
# Define 3 network compartments
#------------------------------------------------------------------------------

fc_bsbs = np.asarray(fc[:n_brainstem, :n_brainstem], dtype = float)
fc_bsctx = np.asarray(fc[:n_brainstem, n_brainstem:], dtype = float)
fc_ctxctx = np.asarray(fc[n_brainstem:, n_brainstem:], dtype = float)

dist_bsbs = np.asarray(dist[:n_brainstem, :n_brainstem], dtype = float)
dist_bsctx = np.asarray(dist[:n_brainstem, n_brainstem:], dtype = float)
dist_ctxctx = np.asarray(dist[n_brainstem:, n_brainstem:], dtype = float)

vol_bsbs = np.asarray(vol[:n_brainstem, :n_brainstem], dtype = float)
vol_bsctx = np.asarray(vol[:n_brainstem, n_brainstem:], dtype = float)
vol_ctxctx = np.asarray(vol[n_brainstem:, n_brainstem:], dtype = float)

#------------------------------------------------------------------------------
# Brainstem–brainstem: remove lower triangle
#------------------------------------------------------------------------------

mask = np.tri(*fc_bsbs.shape, k = 0, dtype = bool) # lower triangle + diag
fc_bsbs_u   = np.where(mask, np.nan, fc_bsbs)
dist_bsbs_u = np.where(mask, np.nan, dist_bsbs)
vol_bsbs_u  = np.where(mask, np.nan, vol_bsbs)

# Remove FC == NaNs
keep = np.isfinite(fc_bsbs_u)

flat_fc_bsbs   = fc_bsbs_u[keep]
flat_dist_bsbs = dist_bsbs_u[keep]
flat_vol_bsbs  = vol_bsbs_u[keep]

#------------------------------------------------------------------------------
# Cortex–cortex: remove lower triangle
#------------------------------------------------------------------------------

mask = np.tri(*fc_ctxctx.shape, k = 0, dtype = bool)
fc_ctxctx_u   = np.where(mask, np.nan, fc_ctxctx)
dist_ctxctx_u = np.where(mask, np.nan, dist_ctxctx)
vol_ctxctx_u  = np.where(mask, np.nan, vol_ctxctx)

# Remove FC == NaNs
keep = np.isfinite(fc_ctxctx_u)

flat_fc_ctxctx   = fc_ctxctx_u[keep]
flat_dist_ctxctx = dist_ctxctx_u[keep]
flat_vol_ctxctx  = vol_ctxctx_u[keep]

#------------------------------------------------------------------------------
# Brainstem–cortex: no symmetry
#------------------------------------------------------------------------------

flat_fc_bsctx   = fc_bsctx.flatten()
flat_dist_bsctx = dist_bsctx.flatten()
flat_vol_bsctx  = vol_bsctx.flatten()

#------------------------------------------------------------------------------
# Stack all data from the three compartments
#------------------------------------------------------------------------------

fc_all   = np.concatenate([flat_fc_ctxctx,   flat_fc_bsctx,   flat_fc_bsbs])
dist_all = np.concatenate([flat_dist_ctxctx, flat_dist_bsctx, flat_dist_bsbs])
vol_all  = np.concatenate([flat_vol_ctxctx,  flat_vol_bsctx,  flat_vol_bsbs])

labels = (
    ['ctx only']     * len(flat_fc_ctxctx) +
    ['bstem to ctx'] * len(flat_fc_bsctx) +
    ['bstem only']   * len(flat_fc_bsbs))

fc_all   = np.asarray(fc_all)
dist_all = np.asarray(dist_all)
vol_all  = np.asarray(vol_all)
labels   = np.asarray(labels, dtype = object)

# Drop NaNs / inf
valid = np.isfinite(fc_all) & np.isfinite(dist_all) & np.isfinite(vol_all)

fc_all   = fc_all[valid]
dist_all = dist_all[valid]
vol_all  = vol_all[valid]
labels   = labels[valid]

color_map = {
    "ctx only":     "#de7eaf",        # cortex–cortex
    "bstem to ctx": "cornflowerblue", # brainstem–cortex
    "bstem only":   "darkgreen",      # brainstem–brainstem
}

#------------------------------------------------------------------------------
# Figure : FC And Distance
#------------------------------------------------------------------------------

plt.figure(figsize=(6,6))
for conn_type in ['ctx only','bstem to ctx', 'bstem only']:
    idx = labels == conn_type
    if conn_type == 'ctx only':
        plt.scatter(
            dist_all[idx],
            fc_all[idx],
            s = 10,
            alpha = 0.1,
            color = color_map[conn_type],
            label = conn_type)
    if conn_type == 'bstem only':
        plt.scatter(
            dist_all[idx],
            fc_all[idx],
            s = 10,
            alpha = 0.8,
            color = color_map[conn_type],
            label = conn_type)
    else:
        plt.scatter(
            dist_all[idx],
            fc_all[idx],
            s = 10,
            alpha = 1,
            color = color_map[conn_type],
            label = conn_type)
    print(conn_type)
    print(spearmanr(dist_all[idx], fc_all[idx]))

sns.despine(top = True, right = True)
plt.tight_layout()
plt.savefig(path_results + 'fc_distance.png', dpi = 300)
plt.show()

#------------------------------------------------------------------------------
# Figure : FC And Volume
#------------------------------------------------------------------------------

plt.figure(figsize = (6, 6))
for conn_type in ['ctx only','bstem to ctx', 'bstem only']:
    idx = labels == conn_type
    if conn_type == 'ctx only':
        plt.scatter(
            vol_all[idx],
            fc_all[idx],
            s = 10,
            alpha = 0.1,
            color = color_map[conn_type],
            label = conn_type)
    if conn_type == 'bstem only':
        plt.scatter(
            vol_all[idx],
            fc_all[idx],
            s = 10,
            alpha = 0.8,
            color = color_map[conn_type],
            label = conn_type)
    else:
        plt.scatter(
            vol_all[idx],
            fc_all[idx],
            s = 10,
            alpha = 1,
            color = color_map[conn_type],
            label = conn_type)
    print(conn_type)
    print(spearmanr(vol_all[idx], fc_all[idx]))

sns.despine(top = True, right = True)
plt.tight_layout()
plt.savefig(path_results + 'fc_volume.png', dpi = 300)
plt.show()

#------------------------------------------------------------------------------
# END